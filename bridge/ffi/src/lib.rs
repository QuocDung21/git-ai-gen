...
struct CleanupDeleteJson {
    reports: Vec<git_ai_core_shared::cleanup::DeleteReport>,
}
...
pub extern "C" fn git_ai_get_status() -> *mut c_char {
    ...
    if let Ok(status_text) = git_ai_core_shared::git::status::get_git_status() {
...
pub extern "C" fn git_ai_cleanup_scan_node_modules(path: *const c_char) -> *mut c_char {
    cleanup_scan(
        path,
        git_ai_core_shared::cleanup::CleanupTarget::NodeModules,
    )
}
...
pub extern "C" fn git_ai_cleanup_scan_build_folders(path: *const c_char) -> *mut c_char {
    cleanup_scan(
        path,
        git_ai_core_shared::cleanup::CleanupTarget::BuildFolders,
    )
}

#[no_mangle]
pub extern "C" fn git_ai_cleanup_scan_devcleaner(path: *const c_char) -> *mut c_char {
    cleanup_scan(path, git_ai_core_shared::cleanup::CleanupTarget::DevCleaner)
}
...
pub extern "C" fn git_ai_cleanup_scan_devcleaner_stream(
    ...
    cleanup_scan_stream(
        path,
        git_ai_core_shared::cleanup::CleanupTarget::DevCleaner,
        callback,
        user_data,
    )
}
...
pub extern "C" fn git_ai_cleanup_delete_paths(paths_json: *const c_char) -> *mut c_char {
    ...
    let reports = git_ai_core_shared::cleanup::delete_paths(&paths);
    json_string(&CleanupDeleteJson { reports })
}

fn cleanup_scan(
    path: *const c_char,
    target: git_ai_core_shared::cleanup::CleanupTarget,
) -> *mut c_char {
    let Some(path) = string_from_c_char(path) else {
        return ffi_error("invalid_path_pointer", "Invalid path pointer");
    };

    let items = git_ai_core_shared::cleanup::scan_folders(Path::new(&path), target)
        .into_iter()
...
fn cleanup_scan_stream(
    ...
    git_ai_core_shared::cleanup::scan_folders_each(Path::new(&path), target, |task| {
...
fn cleanup_scan_stream_cancellable(
    ...
    git_ai_core_shared::cleanup::scan_folders_each_until(Path::new(&path), target, |task| {
...
pub extern "C" fn git_ai_get_diff(path: *const c_char) -> *mut c_char {
    ...
    let diff_output =
        if is_untracked && git_ai_core_shared::git::status::get_diff_head(file_path).is_none() {
            std::fs::read_to_string(file_path).unwrap_or_default()
        } else {
            let mut out = git_ai_core_shared::git::status::get_diff_head(file_path);
            if out.is_none() {
                out = git_ai_core_shared::git::status::get_diff_unstaged(file_path);
            }
            if out.is_none() {
                out = git_ai_core_shared::git::status::get_diff_staged(file_path);
            }
...
pub extern "C" fn git_ai_stage_file(path: *const c_char) {
    ...
    if let Ok(file_path) = c_str.to_str() {
        let _ = git_ai_core_shared::git::status::stage_file(file_path);
    }
}
...
pub extern "C" fn git_ai_unstage_file(path: *const c_char) {
    ...
    let c_str = unsafe { CStr::from_ptr(path) };
    if let Ok(file_path) = c_str.to_str() {
        let _ = git_ai_core_shared::git::status::unstage_file(file_path);
    }
}
...
pub extern "C" fn git_ai_generate_commit_message(diff: *const c_char) -> *mut c_char {
    ...
    let ai_lang = git_ai_core_shared::helper::Helper::get_ai_language_name();
    let prompt = format!(
        "{} {}.\n\nDiff:\n\n{}",
        git_ai_core_shared::constant::Constant::PROMPT_EXPERT,
        ai_lang,
        full_diff
    );
...
pub extern "C" fn git_ai_commit(message: *const c_char) -> *mut c_char {
    ...
    let res = match git_ai_core_shared::git::commit::commit(msg) {
...
pub extern "C" fn git_ai_push() -> *mut c_char {
    setup_env_path();
    let res = match git_ai_core_shared::git::remote::git_push() {
        Ok(_) => "Success".to_string(),
        Err(e) => format!("Error: {}", e),
    };
    CString::new(res).unwrap().into_raw()
...
pub extern "C" fn git_ai_get_workspace_history() -> *mut c_char {
    setup_env_path();
    let history = git_ai_core_shared::helper::Helper::load_history_file("workspace_history.txt");
...
pub extern "C" fn git_ai_add_to_workspace_history(path: *const c_char) {
    ...
    let mut history =
        git_ai_core_shared::helper::Helper::load_history_file("workspace_history.txt");
    history.retain(|p| p != &folder_path);
    history.insert(0, folder_path);
    history.truncate(10);
    let _ =
        git_ai_core_shared::helper::Helper::save_history_file("workspace_history.txt", &history);
}
