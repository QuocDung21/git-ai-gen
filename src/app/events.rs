            ...
            match event::read()? {
                Event::Key(key) => {
                    ...
                    match &app.active_modal {
                        ...
                        ActiveModal::LanguageSelect => {
                            match key.code {
                                ...
                                KeyCode::Char('a') | KeyCode::Char('A') => {
                                    let locales = crate::cli::Locales::new(&app.current_lang);
                                    if let Ok(msg) =
                                        crate::cli::system::handle_lang("auto", &locales)
                                    {
                                        app.status_message = msg;
                                        app.current_lang = Helper::get_ai_language();
                                    }
                        ...
                        ActiveModal::WorkspaceHistory => {
                            ...
                            match key.code {
                                ...
                                KeyCode::Enter => {
                                    if !app.workspace_history.is_empty() {
                                        let selected_path = app.workspace_history
                                            [app.selected_workspace_index]
                                            .clone();
                                        if std::env::set_current_dir(&selected_path).is_ok() {
                                            ...
                                            app.status_message = if is_vi {
                                                "❌ Lỗi: Không thể truy cập thư mục này."
                                                    .to_string()
                                            } else {
                        ...
                        ActiveModal::GitLog => {
                            match key.code {
                                ...
                                KeyCode::Enter => {
                                    ...
                                    {
                                        let hash =
                                            app.commit_logs[app.selected_log_index].hash.clone();
                        ...
                        ActiveModal::KiloModelSelect => {
                            ...
                            if !filtered.is_empty()
                                && app.selected_kilo_model_index >= filtered.len()
                            {
                            ...
                            match key.code {
                                KeyCode::Esc | KeyCode::Char('q') => {
                                    if app.kilo_model_search_mode
                                        || !app.kilo_model_filter.is_empty()
                                    {
                                        app.kilo_model_filter.clear();
                                        app.kilo_model_search_mode = false;
                                        app.selected_kilo_model_index = 0;
                                ...
                                KeyCode::Enter => {
                                    if !filtered.is_empty() {
                                        let idx =
                                            app.selected_kilo_model_index.min(filtered.len() - 1);
                                    ...
                                    match app.selected_git_action {
                                        0 => {
                                            ...
                                            if !app.kilo_ai_enabled {
                                                ...
                                                app.commit_message_preview = if clipboard_msg
                                                    .trim()
                                                    .is_empty()
                                                {
                                                    if app.current_lang == "vi" {
                        ...
                        ActiveModal::GithubDownloadUrlInput => {
                            match key.code {
                                ...
                                KeyCode::Enter => {
                                    ...
                                    if !url.is_empty() {
                                        app.status_message = if app.current_lang == "vi" {
                                            "⏳ Đang tải thông tin repository từ GitHub..."
                                                .to_string()
                                        } else {
                        ...
                        ActiveModal::GithubDownloadTree => {
                            match key.code {
                                ...
                                KeyCode::Char(' ') => {
                                    app.toggle_github_tree_selection(
                                ...
                                KeyCode::Right => {
                                    ...
                                    if let Some(entry) = entry {
                                        if entry.is_dir {
                                            ...
                                            let next_len =
                                                app.get_visible_github_tree_entries().len();
                                ...
                                match key.code {
                                    ...
                                    KeyCode::Up | KeyCode::Char('k') => {
                                        app.github_quickview_scroll =
                                            app.github_quickview_scroll.saturating_sub(1);
                                    }
                                    KeyCode::Down | KeyCode::Char('j') => {
                                        app.github_quickview_scroll =
                                            app.github_quickview_scroll.saturating_add(1);
                                    }
                                    KeyCode::PageUp => {
                                        app.github_quickview_scroll =
                        ...
                        ActiveModal::GithubBranchSelect => {
                            match key.code {
                                ...
                                KeyCode::Up | KeyCode::Char('k') => {
                                    if !app.github_branches.is_empty() {
                                        if app.selected_github_branch_index > 0 {
                                            ...
                                            app.selected_github_branch_index =
                                                app.github_branches.len() - 1;
                                        }
                                    }
                                }
                                KeyCode::Down | KeyCode::Char('j') => {
                                    if !app.github_branches.is_empty() {
                                        if app.selected_github_branch_index
                                            < app.github_branches.len() - 1
                                        {
                                            app.selected_github_branch_index += 1;
                                        } else {
                                ...
                                KeyCode::Enter => {
                                    if !app.github_branches.is_empty()
                                        && app.selected_github_branch_index
                                            < app.github_branches.len()
                                    {
                                        let selected_branch = app.github_branches
                                            [app.selected_github_branch_index]
                                            .clone();
                                        app.status_message = if app.current_lang == "vi" {
                                            format!(
                                                "⏳ Đang chuyển sang nhánh {}...",
                                                selected_branch
                                            )
                                        } else {
                                                    ...
                                                    if let Ok(c_out) = checkout_out {
                                                        if c_out.status.success() {
                                                            if let Err(e) = app.visit_repo_dir() {
                                                                ...
                                                                app.active_modal =
                                                                    ActiveModal::GithubDownloadTree;
                                                                app.status_message = if app
                                                                    .current_lang
                                                                    == "vi"
                                                                {
                                                                    "✅ Đã chuyển nhánh thành công"
                                                                        .to_string()
                                                                } else {
                                                                    "✅ Successfully switched branch"
                                                                    .to_string()
                                                                };
                                                            }
                                                        } else {
                                                        ...
                                                        app.status_message =
                                                            if app.current_lang == "vi" {
                                                                "❌ Không thể chạy lệnh checkout"
                                                                    .to_string()
                                                            } else {
                                                                "❌ Cannot execute checkout command"
                                                                    .to_string()
                                                            };
                                                    }
                                                } else {
                                    ...
                                    KeyCode::Tab => {
                                        ...
                                        if app.commit_input_mode && app.commit_input_text.is_empty()
                                        {
                                            app.commit_input_text =
                                                app.commit_message_preview.clone();
                                        }
                                    }
                                    KeyCode::Enter => {
                                ...
                                StashStep::List => match key.code {
                                    ...
                                    KeyCode::Down | KeyCode::Char('j') => {
                                        if !app.stash_entries.is_empty() {
                                            if app.selected_stash_index
                                                < app.stash_entries.len() - 1
                                            {
                                                app.selected_stash_index += 1;
                                            } else {
                        ...
                        ActiveModal::CommitDiff(_) => {
                            match key.code {
                                ...
                                KeyCode::Up | KeyCode::Char('k') => {
                                    app.commit_diff_scroll =
                                        app.commit_diff_scroll.saturating_sub(3);
                                }
                                KeyCode::Down | KeyCode::Char('j') => {
                                ...
                                KeyCode::PageUp => {
                                    app.commit_diff_scroll =
                                        app.commit_diff_scroll.saturating_sub(15);
                                }
                                KeyCode::PageDown => {
                                ...
                                match key.code {
                                    ...
                                    KeyCode::Char(' ') => {
                                        if !app.files.is_empty()
                                            && app.selected_index < app.files.len()
                                        {
                    ...
                    match key.code {
                        ...
                        KeyCode::Char('r') => {
                            let locales = crate::cli::Locales::new(&app.current_lang);
                            run_cli_command(terminal, || {
                                crate::cli::system::handle_restore(&locales)
                            })?;
                            app.refresh_git_status();
                            app.status_message = if app.current_lang == "vi" {
                ...
                Event::Paste(text) => match &app.active_modal {
                    ActiveModal::ManualCommit => {
                        app.manual_commit_message.push_str(&text);
                    }
                    ActiveModal::GithubDownloadUrlInput => {
